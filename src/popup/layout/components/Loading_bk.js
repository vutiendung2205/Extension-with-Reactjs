...
function Loading(props) {
    ...
    const { addTime, isShow, isShowData } = useSelector(({ layout }) => layout.loading);
    ...
    useEffect(() => {
        ...
        if (!isShow && loadingPercent != 100) {
            clearInterval(interval);
            setLoadingPercent((loadingPercent) => 100);
        }
        return () => clearInterval(interval);
    }, [loadingPercent]);
    useTimeout(() => {
        if (!isShowData && !isShow) {
            dispatch(ActionsLayout.showData());
        }
    }, 100);
    return isShow || !isShowData ? (
