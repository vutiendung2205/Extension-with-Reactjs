...
const renderImage = (item, showInfo) => {
	...
	if (showInfo) {
		if (item.image !== undefined) {
			href = `${PEXGLE_HOST + '/shopify-product-detail?product=' + item.product_id}`;
			src = item.image;
		} else {
		...
		src = chrome.extension.getURL(bestSellingItem);
		href = '#';
		return (
...
function bestSellingItem({ dataItem, userId, length }) {
	...
	const toggleFavorite = item => {
		if (!isFavorite(item) && !item.isFavoriteInProgress) {
			...
			dispatch(Actions.favoriteBestSelling(item.product_id));
		} else if (isFavorite(item) && !item.isFavoriteInProgress) {
			dispatch(Actions.favoriteInProgress(item.product_id));
			dispatch(Actions.unFavoriteBestSelling(item.product_id));
		}
	};
	let isShowInfo = true;
	...
	let productTime = '';
	if (dataItem.published_at != '0000-00-00 00:00:00') {
		productTime = dataItem.published_at;
	}
	let titleContent =
		userId < 0 ? (
			<div>
				...
				<div className={'card id-' + dataItem.product_id}>
					...
					<div className="product-content free-user-video-container">
						<div className="d-flex no-block align-items-center">
							<div className="description" title={dataItem.title} style={{ height: '16px' }}>
								{titleContent}
							</div>
						</div>
						<div
						...
						>
							<span
								...
								title={titleRecentOrder}
							>
								{dataItem.sum_order_7} Recent Orders
							</span>
						</div>
						<div
							className="d-flex no-block align-items-center m-b-10"
							style={{ placeContent: 'center space-between', display: 'flex', lineHeight: '18px' }}
						>
							<span
								className="text-pd-muted m-b-0 font-12 cursor-pointer cursor-pointer f-w-300"
								title="Published at"
							>
								{productTime ? moment(productTime).utc().format('DD/MM/YYYY') : ''}
							</span>
							{countryImage}
						</div>
					...
					<div className="pg-product-content">
						<div className="info-view">
							<div className="video-reaction-panel"></div>
						</div>
						<div style={{ width: '158px', height: '150px' }}>
							{
								<BestSellingChart
									tracking_time_arr={dataItem.analysis_chart}
									length={length}
								></BestSellingChart>
							}
						</div>
